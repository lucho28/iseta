    ...
    <div>
        <div class="perfil_one br">
            <div class="perfil__header">
                ...
                <form method="POST" action="{{route('preceptor.config.setone')}}">
            ...
            <div class="perfil__info">
                ...
                <div class="grid-3 gridx-center gap-4 selec-masivo">
                    <div>
                        ...
                        @if ($anterior)
                            <a class="blue-600"
                                href="{{route('preceptor.cursadas.masivo', ['asignatura' => $anterior->id])}}">{{$anterior->nombre}}</a>
                        @endif
                    </div>
                    <div>
                        <p>Actual</p>
                    ...
                    <div>
                        ...
                        @if ($siguiente)
                            <a class="blue-600"
                                href="{{route('preceptor.cursadas.masivo', ['asignatura' => $siguiente->id])}}">{{$siguiente->nombre}}</a>
                        @endif
                    </div>
                </div>

                <form method="POST" action="{{route('preceptor.cursadas.masivo.post')}}">
                    @csrf
                        ...
                        @if ($carrera->primeraAsignatura())
                            @if ($carrera->id == $asignatura->carrera->id)
                                <p class="text-left gray-700 perfil_dataname-rem"
                                    href="{{route('preceptor.cursadas.masivo', ['asignatura' => $carrera->primeraAsignatura()->id])}}">
                                    {{$carrera->nombre}}
                                </p>
                            @else
