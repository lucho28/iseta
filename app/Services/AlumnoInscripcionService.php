...
class AlumnoInscripcionService
{
    ...
    public function puedeInscribirse($mesa, $alumno)
    {
        ...
        if ($mesa->asignatura->aproboExamen($alumno)) {
            return ['success' => false, 'mensaje' => 'Ya se aprobo esta asignatura'];
        }
        if (! $mesa->asignatura->aproboCursada($alumno)) {
            return ['success' => false, 'mensaje' => 'Aun no se aprobo la cursada de esta asignatura'];
        }
    ...
    public function puedeBajarse($mesa, $examen)
    {
        ...
        if (! $examen) {
            return ['success' => false, 'mensaje' => 'No estas inscripto en esta mesa.'];
        }
    ...
    public function inscribiblesDelAlumno($alumno)
    {
        ...
        foreach ($asignaturas as $asignatura) {
            ...
            if (count($asignatura->mesas) == 0) {
                continue;
            }        // si la asignatura no tiene mesas
            if ($asignatura->aproboExamen($alumno)) {
                continue;
            }    // el alumno ya aprobo el examen
            if (! $asignatura->aproboCursada($alumno)) {
                continue;
            }  // no aprobo la cursada
            ...
            $correlativas = Correlativa::debeExamenesCorrelativos($asignatura, $carrera, $alumno);
            if ($correlativas) {
